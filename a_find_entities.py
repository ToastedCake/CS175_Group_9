...
class Vertex:
    def __init__(self,x,y,z,row,col,parent,g,h):
        self.x = x
        self.y = y
        self.z = z
...
def find_minf_vertex_and_remove(open_List):
    ...
    for vertex in open_List:
        if(vertex.f < minf):
            minf = vertex.f
            minf_Vertex = vertex
    # remove the min f vertex from the open_List
    for i in range(0,len(open_List)):
        if open_List[i] is minf_Vertex:
            open_List.pop(i)
            break
...
def isDestination(v_x,v_y,v_z,dest_x,dest_y,dest_z):
    if(v_x == dest_x and v_y == dest_y and v_z == dest_z):
        return True
    else:
        return False
...
def Astar_search(agent_host,agent_location, target_location):
    ...
    start_vertex = Vertex(s_x,s_y,s_z,center_x,center_y,None,0,h)
    ...
    while(len(open_List)>0):
        ...
        if(isInBoudary(north_succssor_row,north_succssor_col)):
            if(graph[north_succssor_row][north_succssor_col] == 'air'):
                ...
                north_succssor_vertex = Vertex(q.x,q.y,(q.z-1),north_succssor_row,north_succssor_col,q,(q.g+1),h)
        ...
        if(isInBoudary(south_succssor_row,south_succssor_col)):
            if(graph[south_succssor_row][south_succssor_col] == 'air'):
                h = heuristic(q.x,q.y,(q.z+1),d_x,d_y,d_z)
                south_succssor_vertex = Vertex(q.x,q.y,(q.z+1),south_succssor_row,south_succssor_col,q,(q.g+1),h)
        ...
        if(isInBoudary(east_succssor_row,east_succssor_col)):
            if(graph[east_succssor_row][east_succssor_col] == 'air'):
                ...
                east_succssor_vertex = Vertex((q.x+1),q.y,q.z,east_succssor_row,east_succssor_col,q,(q.g+1),h)
        ...
        if(isInBoudary(west_succssor_row,west_succssor_col)):
            if(graph[west_succssor_row][west_succssor_col] == 'air'):
                h = heuristic((q.x-1),q.y,q.z,d_x,d_y,d_z)
                west_succssor_vertex = Vertex((q.x-1),q.y,q.z,west_succssor_row,west_succssor_col,q,(q.g+1),h)
